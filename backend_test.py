            ...
            if mapping_response.status_code == 200:
                ...
                if options:
                    ...
                    if 'DK' in country_codes and 'US' in country_codes:
                        self.log("✅ Country codes correctly parsed from CSV")
                    else:
                        self.log(f"❌ Incorrect country codes: {country_codes}")
                        return False
                else:
                    self.log("❌ No options found for test mapping")
                    return False
            ...
            if empty_mapping_response.status_code == 200:
                ...
                if empty_options:
                    ...
                    if set(empty_countries) == {"DK", "US", "GB"}:
                        self.log("✅ Empty country column defaults to DK,US,GB")
                    else:
                        self.log(f"❌ Incorrect default countries: {empty_countries}")
                        return False
                else:
                    self.log("❌ No options found for empty country test")
                    return False
            else:
                self.log(f"❌ Failed to verify empty country mapping: {empty_mapping_response.status_code}")
                return False
            
            self.log("✅ CSV import with countries working correctly")
            return True
        ...
        try:
            ...
            if create_response.status_code == 200:
                ...
                if created_option.get('country_codes') == ["DK", "US"]:
                    ...
                    self.log(f"❌ Incorrect country codes: {created_option.get('country_codes')}")
                    return False
            else:
                self.log(f"❌ Option creation failed: {create_response.status_code} - {create_response.text}")
                return False
            ...
            if mapping_response.status_code == 200:
                ...
                if test_option:
                    ...
                    if test_option.get('country_codes') == ["DK", "US"]:
                        self.log("✅ Country codes correctly retrieved")
                    else:
                        self.log(f"❌ Retrieved incorrect country codes: {test_option.get('country_codes')}")
                        return False
                else:
                    self.log("❌ Test option not found in mapping")
                    return False
            ...
            if update_response.status_code == 200:
                ...
                if updated_option.get('country_codes') == ["GB"]:
                    self.log("✅ Country codes correctly updated")
                else:
                    self.log(f"❌ Update failed, country codes: {updated_option.get('country_codes')}")
                    return False
            else:
                self.log(f"❌ Option update failed: {update_response.status_code} - {update_response.text}")
                return False
            
            self.log("✅ Option CRUD with countries working correctly")
            return True
