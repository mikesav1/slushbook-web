  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        toast.error('Billede må maks. være 10MB');
        return;
      }
          ...
          {!imagePreview ? (
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-cyan-400 transition-colors">
              ...
              >
                ...
                <div>
                  <span className="text-cyan-600 font-medium">Klik for at uploade</span>
                  <span className="text-gray-500 text-sm"> eller træk og slip</span>
                </div>
                <p className="text-xs text-gray-500">Billede komprimeres automatisk til max 80KB</p>
              </label>
            </div>
          ) : (
          ...
          {imageFile && (
            <div className="mt-3">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={imageOwnershipConfirmed}
                  onChange={(e) => setImageOwnershipConfirmed(e.target.checked)}
                  className="mt-1 w-5 h-5 text-cyan-500 rounded focus:ring-2 focus:ring-cyan-500"
                  required
                />
                <span className="text-sm text-gray-700">
                  Jeg bekræfter, at jeg ejer billedet, og at det ikke er beskyttet af copyright.
                </span>
              </label>
            </div>
          )}
